...
class WikiEventsEmitter {
  ...
  typoedArticlesStatisticsValue: TypoedArticlesStatisticsType =
    new TypoedArticlesStatisticsType();

  mostActiveUserStatisticsSubscription: Subscription;
  mostActiveUserStatisticsValue: MostActiveUserStatisticsType =
    new MostActiveUserStatisticsType();

  wikiEventStatisticsSubscription: Subscription;
  wikiEventStatisticsValue: wikiEventStatisticsType =
    new wikiEventStatisticsType();

  constructor() {
    this.typoedArticlesStatisticsSubscription =
      TypoedArticlesStatistics.subscribe(
        (data: TypoedArticlesStatisticsType) =>
          (this.typoedArticlesStatisticsValue = data)
      );

    this.mostActiveUserStatisticsSubscription =
      MostActiveUserStatistics.subscribe(
        (data: MostActiveUserStatisticsType) =>
          (this.mostActiveUserStatisticsValue = data)
      );
  ...
  GetTypoedArticlesStatistics() {
    return timer(0, 500).pipe(map((_) => this.typoedArticlesStatisticsValue));
  }

  GetMostActiveUserStatistics() {
    return timer(0, 1000).pipe(map((_) => this.mostActiveUserStatisticsValue));
  }

  GetUpdateTypesStatistics() {
    return timer(0, 100).pipe(map((_) => this.wikiEventStatisticsValue));
  }

  GetUserContributionsOverTime(user: string, timeDelay: number) {
    this.userContributionsOverTimeSubscription = wikiUserContributionsOverTime(
      user
    ).subscribe(
      (data: number) => (this.userContributionsOverTimeValue += data)
    );

    return timer(0, timeDelay).pipe(
      map((_) => {
